...
//		dirname(strtok($_SERVER['REQUEST_URI'],'?')).'/login_with_github.php';
		dirname(strtok($_SERVER['REQUEST_URI'],'?')).'/'.basename(__FILE__);

	$client->client_id = $config['github']['client_id'] ?: ''; $application_line = __LINE__;
	$client->client_secret = $config['github']['client_secret'] ?: '';
